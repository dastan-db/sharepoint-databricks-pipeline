...
```mermaid
flowchart TD
    A[SharePoint<br/>Data Sources] --> B[Lakeflow<br/>Connectors]
    B --> C[Unity Catalog<br/>Secure Storage]
    C --> D[Web Application FastAPI]
    
    subgraph D[Web Application FastAPI]
        E[Security Layer<br/>• SecureSQL SQL injection prevention<br/>• Escaped queries & SDK methods]
        F[Services Layer<br/>• SchemaManager Unity Catalog SDK<br/>• JobOrchestrator Databricks Jobs<br/>• ExcelStreaming Job delegation]
        G[Routes Layer FastAPI<br/>• SharePoint connections & pipelines<br/>• Excel streaming configurations]
        E --> F
        F --> G
    end
    
    D --> H[Web UI HTML/CSS/JavaScript<br/>• View SharePoint connections<br/>• Manage ingestion pipelines<br/>• Control streaming jobs]
```
...
flowchart TD
    subgraph A[FastAPI Application]
        B[MCP Client Layer<br/>• call_mcp_tool helper<br/>• Server: databricks]
        C[Services MCP-powered<br/>• WarehouseManager<br/>• UnityCatalog enhanced]
        D[Routes<br/>• /api/catalog/* new<br/>• Existing routes enhanced]
        B --> C
        C --> D
    end
```
